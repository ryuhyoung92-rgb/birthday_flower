...
birthdayBtn.addEventListener('click',function(){
    //태어난 달의 값(속성)이 빈문자열과 같으면 콘솔실행
    if(userMonth.value == '') /* value는 input 태그에서만 사용 */msg_func(1,'block');
    else msg_func(1);
    //error_msg2 {display:block;}
    //error_msg2 객체(변수선언)
    //style 속성(css 선언)
    //display 속성
    //block 값
    ...
    if(userYear.value == '') msg_func(2,'block');
    else msg_func(2);
    // //태어난 일 입력안하고 버튼 클릭시 해당 error_msg 출력하기
    if(userDay.value == '') msg_func(0, 'block');
    else msg_func(0);
})
